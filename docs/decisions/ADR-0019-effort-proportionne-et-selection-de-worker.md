...
When `WorkerInteractionRequirement.structuredOutput` is `REQUIRED`, the worker returns a structured sufficiency signal with its answer. A profile may select `AUTO_BOUNDED`: on an `INSUFFICIENT_NEEDS_DEPTH` signal, `ConversationService` retries once at the next effort rung, no higher than the profile ceiling and only within the same `ResourceBudget`; otherwise it offers a user decision under `ResourceBudget.onExhaustion: REQUEST_HUMAN_DECISION`. The `EffortRungSpec` a `ProviderAccount`/`ProviderPlatform` effort ladder declares carries no cost data of its own, so "the profile ceiling" is `WorkerRequirementProfile.limits.maximumCostClass` read against the same `LOW`/`MEDIUM`/`HIGH` ordinal as `reasoningEffort`: the escalation never resolves a rung whose ordinal exceeds the declared cost ceiling's, regardless of what the ladder itself would otherwise permit. A mandatory contextual risk appraisal for a HIGH or uncertain operation is distinct: it uses a dedicated requirement profile of the same Nestor identity and can only strengthen the risk floor, never authorize an action or waive an obligation.

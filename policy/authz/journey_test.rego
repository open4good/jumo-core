package jumo.authz.journey_test

import data.jumo.authz.journey
import rego.v1
...
test_allows_complete_kind_level_envelope if {
	result := journey.decision with input as valid_input
	result.decision == "ALLOW"
}

test_refuses_action_outside_envelope if {
	result := journey.decision with input as object.union(valid_input, {"action": "cli.session.smoke"})
	result.reasonCodes == ["CAPABILITY_NOT_IN_JOURNEY_ENVELOPE"]
}

test_refuses_inactive_policy_set if {
	result := journey.decision with input as object.union(valid_input, {"activePolicySetRefs": []})
	result.reasonCodes == ["POLICY_SET_NOT_ACTIVE"]
}

test_refuses_missing_step_obligation if {
	result := journey.decision with input as object.union(valid_input, {"satisfiedObligations": []})
	result.reasonCodes == ["JOURNEY_OBLIGATIONS_UNSATISFIED"]
}

test_refuses_wrong_kind_without_naming_a_journey if {
	result := journey.decision with input as object.union(valid_input, {"kind": "ProcessSpec"})
